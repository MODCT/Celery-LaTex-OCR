...
class LatexModel(nn.Module):
    ...
    @torch.no_grad()
    def generate(
        self, start_tokens: torch.Tensor, memory: torch.Tensor, seq_len: int = 256
    ):
        ...
        for _ in range(seq_len):
            ...
            tgt_mask = (
                nn.Transformer.generate_square_subsequent_mask(x.shape[1])
                .to(torch.bool)
                .to(x.device)
            )
            # topk and sample
            logits = self.decoder(x, memory, tgt_mask=tgt_mask)[
                :, -1, :
            ]  # B * vocab_size
    ...
    def generate_batch(self, src: torch.Tensor):
        ...
        # B * 1
        start_tokens = (
            torch.LongTensor([self.bos_token] * src.shape[0])
            .unsqueeze(1)
            .to(src.device)
        )
        # B * T
    ...
    def forward(
        self, src: torch.Tensor, tgt: torch.Tensor = None, generate: bool = False
    ) -> torch.Tensor:
        """autoregressive
        ...
        if not generate:
            ...
            # (T-1) * (T-1)
            tgt_mask = (
                nn.Transformer.generate_square_subsequent_mask(tgt.shape[1])
                .bool()
                .to(tgt.device)
            )
...
def get_encoder(
    in_chans=1,
    next_depths: List[int] = [3, 3, 9, 3],
    next_dims: List[int] = [64, 128, 256, 512],
...
def build_model(
    ...
    assert (
        model_name in __supported_encoder__

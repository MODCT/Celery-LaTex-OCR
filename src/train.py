...
def train(
    ...
    for it, (img, tgt) in enumerate(tqdm(data_loader)):
        ...
        if (it + 1) % accum_grd_step == 0:
            ...
            if (it + 1) % eval_freq == 0:
                model.eval()
                res = evaluate(
                    model, device, conf, max_batch=conf.eval_batch, sample_freq=20
                )
                ...
                pred_sample_str = conf.pred_sample_md_template.format(
                    true_sample=res["sample_true"], pred_sample=res["sample_pred"]
                )
                tb.add_text("eval/predict_sample", pred_sample_str, sample_count)
                model.train()

        tb.add_scalar(
            "train/max_memory_alloc",
            torch.cuda.max_memory_allocated(device) / 1000 / 1000,
            sample_count,
        )
    ...
    print((f"epoch: [{epoch}], " f"bleu: [{res['bleu']:.4f}]"))
    return res["bleu"]


def main(conf: Config):
    train_transforms = get_transforms(
        conf.min_img_size, conf.max_img_size, mode="train"
    )
    ...
    scheduler = optim.lr_scheduler.StepLR(
        optimizer,
        step_size=int(conf.grad_adj_freq / conf.accum_grd_step),
        gamma=conf.grad_gamma,
        verbose=False,
    )
    ...
    if conf.load_resume:
        ...
        if os.path.exists(conf.resume_optim):
            optimizer.load_state_dict(
                torch.load(conf.resume_optim, map_location=device)
            )
...
if __name__ == "__main__":
    ...
    args = parser.parse_args(
        [
            "-c",
            "src/config/config_convnext.json",
        ]
    )
